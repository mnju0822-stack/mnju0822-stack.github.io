...
- **공통으로 도드라지는 단어**:  "night", 
- **한 작품에만 도드라지는 단어** : 
- Frankenstein에서는 miserable, death, father과 같은 단어가 도드라진다. 그리고 인간으로서의 존재론적 고뇌를 다루는 작품의 특성을 잘 보여준다. 반면, Dracula 는 어떤 작품인지 읽어보지 않아 내용은 잘 알지 못하지만, Frankenstein과 비교했을 때 보다 역동적이고 외부의 사건이 중심이 되는 이야기인 것 같다. came, go, took, went와 같은 동작 동사들이 많이 사용되었다. 
