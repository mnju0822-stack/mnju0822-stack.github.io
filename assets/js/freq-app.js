...
form.addEventListener("submit", (e) => {
    e.preventDefault();                 // 새로고침 막기, 중괄호로 결과를 묶어주기
    const text    = textBox.value;
    const targets = targetBox.value.split(",").map(s => s.trim());
    const counts  = targets.map(t => countChar(text, t)); //글자수 반환해 배열 
    drawList(targets, counts);
    drawTop(targets, counts);
}); // 하나의 명령 단위로 끝나게 되어서 세미콜론 찍어주어야 

function drawList(targets, counts) {    // 새로만든 배열 타겟츠로
    list.textContent = "";              // #freq-list 기존 내용 비우기
    for (let i = 0; i < targets.length; i++) {
        const li = document.createElement("li"); //콘텐트 만들기
        li.textContent = `'${targets[i]}': ${counts[i]}번`;
        list.appendChild(li);
    }
}

function drawTop(targets, counts) {  // 1등 글자 
    let maxIdx = 0;
